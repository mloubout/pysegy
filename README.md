...
Install the optional viewer dependencies and launch the app on the computer that
stores the dataset:
...
`pysegy-viewer` starts the Streamlit interface on `127.0.0.1` and reads files
directly from the local filesystem. Dataset contents are not uploaded or copied
to a remote service. The viewer provides survey summaries and diagnostics,
source and receiver geometry, file and trace headers, and bounded image or
wiggle displays for individual and comparative gathers. Scan metadata can be
cached locally and cleared from the sidebar.

The **Open file…** button uses the operating system chooser. On Linux this
requires Zenity or KDialog; manual path entry remains available for files,
directories, mounted volumes, and network paths.
